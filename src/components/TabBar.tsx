    ...
    <>
      ...
      <header className="sticky top-0 z-40 hidden border-b border-line bg-paper/90 backdrop-blur sm:block">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-6 py-3">
          <Link href="/vision" className="font-hand text-2xl text-ink">
            Board
          </Link>
              ...
              return (
                <Link
                  ...
                  className={`rounded-full px-4 py-1.5 text-sm transition ${
                    active ? "bg-ink text-paper" : "text-ink-soft hover:bg-black/5 hover:text-ink"
                  }`}
                >
                  {tab.label}
                </Link>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-40 flex border-t border-line bg-paper/95 backdrop-blur sm:hidden">
        {TABS.map((tab) => {
          ...
          return (
            ...
            >
              <span
                className={`h-1.5 w-1.5 rounded-full ${active ? "bg-ink" : "bg-transparent"}`}
              />
              {tab.label}
            </Link>
          );
